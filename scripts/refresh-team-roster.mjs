...
const transactionResult = await refreshTransactions({ file: transactionFile, sourceUrl: process.env.SEAHAWKS_TRANSACTIONS_URL || DEFAULT_TRANSACTION_SOURCE });
const transactionStore = transactionResult.store;
await refreshInjuryReports({ file: path.join(root, "src/data/team/injuries.json"), sourceUrl: process.env.SEAHAWKS_INJURIES_URL || DEFAULT_INJURY_SOURCE });
