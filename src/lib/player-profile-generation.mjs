...
export function validateGeneratedProfile(output, player) {
  const errors=[],copy=[output?.bio,output?.seasonOverview,...(Array.isArray(output?.careerHighlights)?output.careerHighlights:[])].filter((value)=>typeof value==="string").join(" ");
