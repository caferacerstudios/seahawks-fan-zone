...
export function validateGeneratedProfile(output, player) {
  ...
  const known=new Set([...(player.editorialFacts||[]).map(x=>x.id),...(player.careerFacts?.sourceFacts||[]).map(x=>x.id)]); for(const id of [...(output?.usedFactIds||[]),...(output?.careerHighlights||[]).flatMap(x=>x.factIds||[]),...(output?.seasonOverview?.bullets||[]).flatMap(x=>x.factIds||[])]) if(!known.has(id)) errors.push(`unknown fact ID: ${id}`);
  const normalized=(s)=>String(s||"").toLowerCase().replace(/[^a-z0-9 ]/g,"").replace(/\s+/g," ").trim(), seen=new Set(); for(const sentence of copy.split(/[.!?]+/).map(normalized).filter(Boolean)){if(seen.has(sentence))errors.push("duplicate sentence");seen.add(sentence);} const bulletKeys=bullets.map(normalized); if(new Set(bulletKeys).size!==bulletKeys.length)errors.push("duplicate bullet");
