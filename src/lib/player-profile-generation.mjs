...
const normalizedFactField = (field) => String(field || "")
  .replace(/([a-z0-9])([A-Z])/g, "$1_$2")
  .toLowerCase()
  .replace(/[^a-z0-9]+/g, "_")
  .replace(/^_+|_+$/g, "");
const factIdField = (field) => normalizedFactField(field).replace(/_/g, "-");
...
export function createSeasonStatisticFacts(playerId, season, scope, statistics = {}) {
  ...
  return Object.entries(statistics).filter(([, value]) => value != null).map(([rawField,value]) => {
    const field = normalizedFactField(rawField);
    return {
      id:`season-stat:${playerId}:${season}:${factIdField(field)}`,
      type:"seasonStat",
      season,
...
export function createCareerStatisticFacts(playerId, career = {}) {
  const facts = [];
  for (const [rawField, value] of Object.entries(career?.careerTotals || {})) {
    if (value == null || typeof value === "object") continue;
    const field = normalizedFactField(rawField);
    facts.push({id:`career-total:${playerId}:${factIdField(field)}`,type:"careerTotal",field,value});
  }
  for (const [rawField, high] of Object.entries(career?.careerHighs || {})) {
    if (!high || high.value == null) continue;
    const field = normalizedFactField(rawField);
    facts.push({id:`career-high:${playerId}:${factIdField(field)}`,type:"careerHigh",field,value:high.value,season:high.season ?? null});
  }
  for (const season of career?.recentSeasons || []) {
    if (season?.season == null) continue;
    for (const [rawField, value] of Object.entries(season)) {
      if (["season", "postseason"].includes(rawField) || value == null || typeof value === "object") continue;
      const field = normalizedFactField(rawField);
      facts.push({id:`recent-season:${playerId}:${season.season}:${factIdField(field)}`,type:"recentSeason",season:season.season,scope:season.postseason ? "postseason" : "regular",field,value});
    }
  }
  return facts;
}
...
const factValue = (fact, field) => normalizedWords(fact?.value?.[field] ?? fact?.[field]);
export function supportsClaim(player, claim, copy = "") {
  const facts = suppliedFacts(player);
  switch (claim) {
    case "signed":
      return facts.some((fact) => fact?.type === "acquisition" && /\b(sign|signing|signed|free agent|free agency)\b/.test(factValue(fact, "method")));
    case "joined":
      return facts.some((fact) => fact?.type === "acquisition" || fact?.type === "teamHistory");
    case "drafted":
      return facts.some((fact) => fact?.type === "draftSelection");
    case "traded":
      return facts.some((fact) => fact?.type === "acquisition" && /\btrade(?:d)?\b/.test(factValue(fact, "method")));
    case "starter":
      return facts.some((fact) => fact?.type === "currentRole" && /\b(starting|starter|first string)\b/.test(factValue(fact, "role")));
    case "captain":
...
export function validateGeneratedProfile(output, player) {
  ...
  const checks=[
    [/\b(?:signed|signing)\b/i,"signed"],
    [/\bjoined\b/i,"joined"],
    [/\b(?:drafted|draft selection)\b/i,"drafted"],
    [/\b(?:traded|acquired (?:in|via) (?:a )?trade)\b/i,"traded"],
