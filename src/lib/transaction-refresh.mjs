    ...
    for (const row of block.matchAll(/nfl-c-transactions-report__date[^>]*>(\d{2})\/(\d{2})<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>/gi)) {
      ...
      statements.forEach((statement, index) => {
        ...
        records.push({
          ...
          sourcePublisher: "Seattle Seahawks", sourceUrl: DEFAULT_TRANSACTION_SOURCE, updateStatus: "Official",
        });
      });
    }
  }
...
export function reconcileTransactions(store, fetched, { now = new Date(), sourceUrl = DEFAULT_TRANSACTION_SOURCE } = {}) {
  ...
  return { ...store, records, asOf: now.toISOString(), sourcePublisher: "Seattle Seahawks", sourceUrl,
    sourceNote: `Official Seahawks transaction log verified${latestDate ? ` through ${latestDate}` : ""}; records are event-based and may include multiple moves for one player.` };
}
...
export async function refreshTransactions({ file, fetchImpl = globalThis.fetch, now = new Date(), sourceUrl = DEFAULT_TRANSACTION_SOURCE, warn = console.warn, log = console.log } = {}) {
  ...
  try {
    const response = await fetchImpl(sourceUrl, { headers: { Accept: "text/html", "User-Agent": "SeahawksFanZone transaction refresh" }, signal: AbortSignal.timeout(15_000) });
