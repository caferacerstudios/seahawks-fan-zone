...
const expectedSlug = "daily-seahawks-2026-09-10-week-1-showed-the-seahawks-biggest-early-season-question-can-their-secondary-depth-hold-up";
const expectedHref = `/news/${expectedSlug}/`;
const fixtureHero = "/images/news/newsroom-field.svg";
const sources = [
  { label: "Fixture Seahawks source", url: "https://www.seahawks.com/news/fixture" },
...
function generatedArticle(day, { slug, featured = false, updatedAt } = {}) {
  ...
  return {
    slug: slug ?? `daily-seahawks-${publicationDay}-fixture-story`,
    headline: day === "10" ? "Week 1 showed the Seahawks’ biggest early-season question: Can their secondary depth hold up?" : `Fixture story for September ${Number(day)}`,
    dek: "An isolated fixture used to verify the rendered homepage news ordering.",
    publishedAt, updatedAt: updatedAt ?? publishedAt,
    author: "Seahawks Fan Zone", category: "Analysis", tags: ["Testing"], season: 2026, opponent: null,
