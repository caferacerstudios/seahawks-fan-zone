...
const team = { id: 31, abbreviation: "SEA", full_name: "Seattle Seahawks" };
...
function snapshot(t) {
  ...
  writeJson(path.join(selected, "seahawks.json"), { ...base, games: [game] });
  ...
  writeJson(path.join(target, "seahawks.json"), { ...base, games: [game] });
  ...
  const rehash = () => {
    for (const name of ["seahawks.json", "players.json", "standings.json", "gameRecaps.json"]) manifest.files[name] = createHash("sha256").update(fs.readFileSync(path.join(selected, name))).digest("hex");
...
function collectorProject(t) {
  ...
  writeJson(path.join(directory, "src/data/nfl/seahawks.json"), { season: 2026, games: [game] });
...
test("strict collection fails on a provider error even with a valid cached schedule", (t) => {
  ...
  const previous = fs.readFileSync(path.join(setup.directory, "src/data/nfl/seahawks.json"), "utf8");
  ...
  assert.equal(fs.readFileSync(path.join(setup.directory, "src/data/nfl/seahawks.json"), "utf8"), previous);
});
...
test("collector uses documented phase filters and reports one coherent fresh snapshot", (t) => {
  ...
  for (const name of ["seahawks.json", "players.json", "standings.json"]) {
