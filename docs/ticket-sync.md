...
The EventSpy collector is a separate disabled-by-default all-games one-shot. Stable commands are `node scripts/tickets/eventspy-collector.mjs validate`, `rehearse`, `collect`, and `status --json`. Validate/status make no browser or network calls. Rehearse requires `EVENTSPY_REHEARSAL_ROOT` pointing at a disposable root and uses the canonical sanitized fixture copied into the collector image. Collect runs sequentially only with `EVENTSPY_ENABLED=true`. Configuration is `EVENTSPY_RUNTIME_ROOT`, `EVENTSPY_HISTORY_ROOT`, validated `EVENTSPY_USER_AGENT`, `EVENTSPY_MAX_ATTEMPTS_PER_GAME_PER_PACIFIC_DAY=2`, and `EVENTSPY_NAVIGATIONS_PER_GAME_PER_RUN=1`; arbitrary game/URL overrides are rejected. Complete batches exit 0. Completed partial batches retain the structured `EVENTSPY_BATCH_PARTIAL_FAILURE` outcome and exit 2, which the systemd unit accepts so successful game histories are synchronized. Fatal setup, configuration, and browser-launch failures exit 1 and do not trigger synchronization. Histories live below `games/<safe-game-id>/current`, and missing/corrupt history falls back only to that game’s prior published history. This does not enable the pending direct StubHub adapter.
