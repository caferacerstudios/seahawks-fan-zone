...
EventSpy is a separate, disabled-by-default collector and market-observation contract. It recognizes only the exact authorized URL and game `1392216`. A durable Pacific-day ledger reserves each attempt before the collector's single navigation; failed attempts count and a third attempt makes no request. The browser path waits for the existing Recharts display and parses visible latest-point tooltip text. It never derives values from coordinates, pixels, colors, OCR, or bundles. Sanitized HTML/payload fixtures exercise the same normalization without network access.

The collector publishes only normalized history, retained for 45 days, through immutable versions and an atomic `current` pointer. The Ticketmaster pipeline reads that local history and attaches it only to game `1392216`; it never contacts EventSpy. EventSpy observations are not listings or direct marketplace integrations and do not enable StubHub.
