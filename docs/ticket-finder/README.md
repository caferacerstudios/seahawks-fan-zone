...
EventSpy is a separate market-observation contract and sanitized-fixture parser. It recognizes only the exact URL and SFZ game mapping recorded in [`provider-rights/EventSpy.md`](provider-rights/EventSpy.md). The runtime event contract may contain a `marketObservations` array populated by the separately controlled twice-daily observer; publication and browser validation fail closed on identity, price, fee-basis, and timestamp errors. The dashboard renders only stored samples, preserves gaps, and never creates fixture or interpolated points. This repository still performs no EventSpy retrieval and does not add EventSpy to the provider registry or Ticketmaster sync job. EventSpy observations are neither listings nor Ticketmaster Discovery ranges and do not approve or enable StubHub.
