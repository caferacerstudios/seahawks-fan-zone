...
The browser performs one exact top-level navigation per authorized game in sequential order. It targets the “Lowest Ticket Price History” heading and chart application, advances with ArrowRight, retains the greatest visible timestamp, stops after three unchanged timestamps (absolute cap 200), and parses ordinary text for four nullable marketplaces. The collector publishes only normalized history, retained for 45 days/100 samples per game, through per-game immutable versions and an atomic `current` pointer. The pipeline resolves and attaches each game independently; EventSpy observations are not listings or direct marketplace integrations and do not enable StubHub.
