...
| EventSpy | APPROVED: accountless retrieval of one exact public URL only | Displayed lowest-price observations; not inventory or provider ranges | Not established | Separate summary plus four nullable marketplace series values, USD, timestamps, optional sections | Unknown; verify checkout | No more than two Pacific-day attempts | Twice daily | 45 days normalized only | 45 days; no backfill or raw content | Not approved | Plain-text attribution and exact source link | Operator permission evidence held outside repository; `provider-rights/EventSpy.md` | IMPLEMENTED, DISABLED BY DEFAULT — isolated collector and local-history pipeline |
