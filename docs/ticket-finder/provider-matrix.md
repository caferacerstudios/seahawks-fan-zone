...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Ticketmaster | APPROVED: Discovery API | Event summary only | Not established | Event ID/name/URL, attractions, venue, dates/timezone, status/sales, classifications, genuine price ranges/all-inclusive flag | Not established; never assumed fee-complete | Account quota | 15-minute dev poll | 60-minute last-good window | No raw response/history | Not established | Plain-text provider name | Operator approval; `provider-rights/Ticketmaster.md` | ENABLED only by operator env |
...
| TicketNetwork | PENDING | PENDING (event-level or listing-level) | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | Blocked checklist: `provider-rights/TicketNetwork.md` (not approval evidence) | DISABLED — rights summary incomplete |
