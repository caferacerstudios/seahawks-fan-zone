...
Run `deployment/prepare-eventspy-dev.sh --browser-image <reviewed-image@sha256:digest>` to print reviewable guarded preparation commands; it makes no host changes itself. Keep `EVENTSPY_ENABLED=false` for `validate`, `status --json`, and the offline image smoke (`node scripts/tickets/eventspy-image-smoke.mjs` in the promoted image with network disabled). Verify UID 1000, read-only root, dropped capabilities, no-new-privileges, bounded `/tmp`, no ports, no persistent browser profile, and only `/srv/eventspy` writable.

Rehearse into an explicitly disposable root with networking disabled and keep the timer disabled. The operator-only live precheck is `node scripts/tickets/eventspy-collector.mjs diagnose --game 1392216`; it requires `EVENTSPY_ENABLED=true`, accepts only an authorized canonical-manifest game, consumes one normal Pacific-day attempt immediately before its single navigation, and never retries. It emits only bounded identity diagnostics. Require `EVENTSPY_COLLECTION_SUCCESS`, a validated history observation for that game, exactly one ticket synchronization, the matching published event observation, the same sanitized browser-runtime observation, and a dev page that has left the not-collected state. Any failed check is **NOT READY**.

Only after that precheck may the operator run the first full batch. Exit 0 means a complete successful batch. Exit 2 means a completed partial batch with at least one success. Exit 3 means attempted games with zero successes and blocks activation. Exit 4 means no navigation because all eligible rows were unavailable or daily-limited and also blocks activation. Fatal configuration, launch, manifest, or setup errors exit 1. The collector unit synchronizes tickets exactly once after completed exits 0, 2, 3, or 4 and preserves the original exit; it does not use `OnSuccess`. Enable `sfz-eventspy-collector@dev.timer` only after a real observation is stored, published, served, and read successfully and the full-batch acceptance is approved. The timer targets 08:00 and 20:00 America/Los_Angeles with a small randomized delay. The per-game application ledger—not the timer or host flock—is authoritative.

Monitor with the service/timer status and journal. Pre-extraction results distinguish `EVENTSPY_HTTP_STATUS_REJECTED`, `EVENTSPY_UNAPPROVED_REDIRECT`, `EVENTSPY_ACCESS_BLOCKED`, `EVENTSPY_CONSENT_BLOCKED`, `EVENTSPY_HEADING_MISSING`, `EVENTSPY_MATCHUP_MISMATCH`, `EVENTSPY_DATE_MISSING`, `EVENTSPY_DATE_MISMATCH`, `EVENTSPY_CHART_MISSING`, `EVENTSPY_TOOLTIP_UNAVAILABLE`, and `EVENTSPY_TOOLTIP_INVALID`. Logs contain no page body, raw HTML, response body, headers, cookies, credentials, storage, or tooltip text. A failed attempt consumes a slot and never erases previous history. Do not retry that day beyond the remaining ledger slot.

For a genuinely corrupt per-game ledger, first disable the timer, preserve it for audit, determine from the journal how many Pacific-day navigations occurred, and wait until the next Pacific day before creating a reviewed schema-2 ledger atomically. Never delete it and immediately rerun. Rollback disables the timer and restores the prior reviewed image/config while retaining last-valid normalized histories. Takedown disables timer and collector, removes the web history mount after review, and deletes only expired normalized versions; raw pages, screenshots, browser profiles, and EventSpy graph history are never stored. No backfill is performed.
