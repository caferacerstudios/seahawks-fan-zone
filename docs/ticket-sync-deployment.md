...
Rehearse into an explicitly disposable root with networking disabled. Confirm the timer remains disabled. Then enable collection for one authorized live precheck, inspect bounded outcomes and each normalized `games/<id>/current/history.json`, run ticket publication, validate all index/event files and the dev page, and only then enable `sfz-eventspy-collector@dev.timer`. It targets 08:00 and 20:00 America/Los_Angeles with a small randomized delay. The per-game application ledger—not the timer or host flock—is authoritative.
